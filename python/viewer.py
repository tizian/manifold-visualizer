...
class ManifoldViewer(Screen):
    def __init__(self):
        ...
        self.modes = {}
        mode_dicts = [
            (ModeType.Raytracing, RaytracingMode),
        ...
        scene_selection.set_fixed_width(260)
    ...
    def keyboard_event(self, key, scancode, action, modifiers):
        ...
        if key == glfw.KEY_1 and action == glfw.PRESS:
            self.mode_selection_cb(ModeType.Raytracing)
            self.mode_selection.set_selected_index(ModeType.Raytracing)
            return True
    ...
    def draw(self, ctx):
        if self.mode == ModeType.Raytracing:
            self.set_caption("Manifold 2D Visualization - Raytracing")
