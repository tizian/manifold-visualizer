    ...
    def draw(self, ctx, scene):
        ...
        if self.sms_mode:
            ...
            for k, path in enumerate(self.intermediate_paths):
                draw_intermediate_path_lines(ctx, path, nvg.RGB(int(r[k]), int(g[k]), 0), 2*s)

        if self.show_normals:
            for t in np.linspace(0, 1, 11):
                it = self.scene.first_specular_shape().sample_position(t)
                draw_arrow(ctx, it.p, it.n, nvg.RGB(50, 50, 50), scale=s*0.8, length=s*0.1, head_scale=s*1.0)
